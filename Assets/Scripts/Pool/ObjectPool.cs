...
namespace Pool
{
    public class ObjectPool : GameObjectSingleton<ObjectPool>
    {
        ...
        private void PreparePool()
        {
            ...
            for (int i = 0; i < assetManager.EnemyPrefabs.Count; i++)
            {
                Prepare(assetManager.EnemyPrefabs[i].Prefab,assetManager.EnemyPrefabs[i].PoolCount);
            }
            for (int i = 0; i < assetManager.TowerPrefabs.Count; i++)
            {
                Prepare(assetManager.TowerPrefabs[i].Prefab,assetManager.TowerPrefabs[i].PoolCount);
            }
        }
